...
function AddAmbulance() {
  ...
  const { addAmbulance, loading } = useAmbulanceContext();
  ...
  const validateField = (name, value) => {
    ...
    switch (name) {
      case 'name':
        if (!value) error = 'Lab Test name is required';
        break;
  ...
  const handleSubmit = async () => {
    ...
    try {
      await addAmbulance(ClinicDetailsMain, BasicDetail, details);
  ...
  return (
    <>
      ...
      <Card width='100%' style={{ marginLeft: 40, marginRight: 80, marginTop: 20, marginBottom: 20 }}>
        <Box display="flex" justifyContent="space-between" alignItems="center">
          ...
          <Box display='flex' alignItems='center'>
            <Tooltip title="Back to Ambulance">
      ...
      <FormSection title="Profile" fields={AmbulanceProfileFields} />
